...
void Adafruit_ADS1015::startOneShot_Differential(adsDiffMux_t regConfigDiffMUX)
{
	// Initial Differential non-contunuous read primes the conversion buffer with a valid reading
	// so that the initial interrupts produced a correct result instead of a left over 
	// conversion result from previous operations.
	...
	uint16_t config = ADS1X15_REG_CONFIG_CQUE_1CONV | // Comparator enabled and asserts on 1 match
		ADS1X15_REG_CONFIG_CPOL_ACTVLOW | // Alert/Rdy active low   (default val)
		ADS1X15_REG_CONFIG_MODE_SINGLE;   // Continuous conversion mode
...
/**************************************************************************/
/*!
	@brief  Sets up OneShot continous coversion operation between the
			P (AIN1) and N (AIN3) input, causing the
			ALERT/RDY pin to assert (go from high to low) each time a conversion
			completes. Pin stays low for 8 micro seconds (per the datasheet)
*/
/**************************************************************************/
void Adafruit_ADS1015::startOneShot_Differential_0_1() {
	startOneShot_Differential(DIFF_MUX_0_1);                             // AIN0 = P, AIN1 = N
}
...
*/
/**************************************************************************/
void Adafruit_ADS1015::startOneShot_Differential_0_3() {
	startOneShot_Differential(DIFF_MUX_0_3);                             // AIN0 = P, AIN1 = N
}
...
*/
/**************************************************************************/
void Adafruit_ADS1015::startOneShot_Differential_1_3() {
	startOneShot_Differential(DIFF_MUX_1_3);                             // AIN0 = P, AIN1 = N
}
...
*/
/**************************************************************************/
void Adafruit_ADS1015::startOneShot_Differential_2_3() {
	startOneShot_Differential(DIFF_MUX_2_3);                             // AIN0 = P, AIN1 = N
}

/**************************************************************************/
/*!
	@brief  Sets up one shot coversion operatoin, causing the
			ALERT/RDY pin to assert (go from high to low) each time a conversion
			completes. Pin stays low for 8 micro seconds (per the datasheet)
*/
/**************************************************************************/
void Adafruit_ADS1015::startOneShot_SingleEnded(uint8_t channel)
{
	// Initial single ended non-contunuous read primes the conversion buffer with a valid reading
	// so that the initial interrupts produced a correct result instead of a left over 
	// conversion result from previous operations.
